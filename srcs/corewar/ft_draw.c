...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/06/26 11:43:41 by achirat           #+#    #+#             */
/*   Updated: 2018/06/26 11:43:43 by achirat          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_draw_joueurs(t_data data, t_draw draw, t_proc **begin)
{
	int	i;
	...
	ft_draw_general_info(data, draw, begin);
}
...
void	ft_handle_cycle_speed(t_data data, t_draw *draw,
								t_proc **begin, char ch)
{
	int	tmp;
...
void	ft_draw_cycle(t_draw *draw, t_data *data)
{
	...
	ft_redimension(draw);
	if (draw->col >= 64 * 3 + 50)
		ft_draw_joueurs(*data, *draw, &data->proc_list);
	i = 0;
	while (i < MEM_SIZE)
	{
		if (i % 64 == 0)
	...
	if (draw->col >= 64 * 3 + 50)
		ft_draw_general_info(*data, *draw, &data->proc_list);
	if (data->dump_val && data->cyc_tot == data->dump_val)
	{
		getch();
		endwin();
	}
	ft_handle_key_input(*data, draw, &data->proc_list);
	refresh();
}
