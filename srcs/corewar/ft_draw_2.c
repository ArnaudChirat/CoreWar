...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/06/26 11:43:52 by achirat           #+#    #+#             */
/*   Updated: 2018/06/26 11:43:55 by achirat          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_draw_general_info(t_data data, t_draw draw, t_proc **begin)
{
	...
	printw("Cycles since last check= %-4d", data.cy);
	move(4 + 64, draw.col / 2 - (64 / 2 * 3));
	printw("Total cycles = %-4d", data.cyc_tot);
	move(5 + 64, draw.col / 2 - (64 / 2 * 3));
	printw("Cycle to die = %-4d", data.ctd);
	move(6 + 64, draw.col / 2 - (64 / 2 * 3));
	printw("Check = %-2d", data.check);
