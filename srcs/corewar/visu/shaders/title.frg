...
vec2	map(vec3 eye, vec3 rd)
{
	...
	while (i < max_it)
	{
		...
		dst = 
			max(max(-cube(roty(rotx(rotz(vec3(ray.xy, repeat1(ray.z, 50))))), 40. + cos(u.time) * 20.),
			-cube(rotz(rotx(rotx(vec3(bfk(ray.xyx, 0.1).xy, repeat1(ray.z, 75))))), 40. + sin(u.time) * 20.)),
...
void	main()
{
	...
	*/
	...
	vec3 tar = vec3(0., 0.0, 3. + u.time * 60.);
	vec3 eye = vec3(0., 0.0, u.time * 60.);
	vec3 up = vec3(0.0, 1.0, 0.0);
	...
	vec2 ret = map(eye, rd);
	fragColor = vec4(1. - ret.x * 6.6,1. - ret.y * 6.6,1. - ret.y * 6.6, 1.0);
	fragColor.xyz = pow(fragColor.xyz, vec3(30.5 + cos(u.time * 2.) * 0.2));
	fragColor = vtxColor;
}
